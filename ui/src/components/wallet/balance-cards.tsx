...
export function BalanceCards({
  ...
  return (
    <div className="space-y-4">
      ...
      {activated && sandbox && (
        <section className="flex flex-col rounded-2xl border border-dashed border-border bg-card p-6">
          <div className="flex items-center gap-2">
            ...
            <p
              className="font-mono text-xs uppercase tracking-widest text-muted-foreground">{t('balance.sandbox.label')}</p>
          </div>
      ...
      {activated && (
        <Link href="/gambling/responsible"
              className="flex items-center gap-2 text-sm font-medium text-brand-700 hover:underline [@media(pointer:coarse)]:min-h-11 [@media(pointer:coarse)]:min-w-11">
